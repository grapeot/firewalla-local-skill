...
- Local raw output handling.

Collectors are stateless functions. They build read-only remote Redis commands, parse `redis-cli --raw` output, apply command-level parameters, and return JSON-serializable objects with real local values preserved.
...
Collectors preserve real device names, IPs, MACs, domains, alarm messages, and flow fields. This is required for reliable joins and practical network investigation. Generated artifacts belong in git-ignored local paths. Public docs and tests use fake fixtures rather than transformed live data.
...
The attribution output includes a `device_summary` field with human-readable device identity information from the device inventory.
...
Analysis commands such as `cluster`, `device-summary`, `attribute`, and `active-devices` read these JSON artifacts and preserve collection metadata in their own output.
...
**Offline tests** (`-m "not live"`): Run without Firewalla connectivity. Cover dry-run behavior, allowlist enforcement, mutation rejection, config parsing, local raw output, timestamp filtering, alarm attribution rules, identity conflict handling, and JSON schema conformance. Use mock Redis responses.
